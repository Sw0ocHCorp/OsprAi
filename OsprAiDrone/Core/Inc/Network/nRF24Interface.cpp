...
HAL_StatusTypeDef nRF24Interface::EnterInRXMode() {
	...
	CLearTxSensorBuffer();
	HAL_StatusTypeDef status= WriteSPIData(EN_RXADDR_REG, 0x01, W_REGISTER_CMD);
	uint8_t mode=0b00001011;
	status= WriteSPIData(CONFIG_MODE_REG, mode, W_REGISTER_CMD);
	return status;
}
...
HAL_StatusTypeDef nRF24Interface::EnterInTXMode() {
	...
	HAL_StatusTypeDef status= WriteSPIData(EN_RXADDR_REG, 0x00, W_REGISTER_CMD);
...
HAL_StatusTypeDef nRF24Interface::CLearTxSensorBuffer() {
	uint8_t data= 0x00;
	return WriteSPIData(FLUSH_TX_CMD, data, W_REGISTER_CMD);
}

HAL_StatusTypeDef nRF24Interface::CLearRxSensorBuffer() {
	uint8_t data=0x00;
	return WriteSPIData(FLUSH_RX_CMD, data, W_REGISTER_CMD);
}
