...
private:
	...
	HAL_StatusTypeDef getAccelData(double *linAccelVect, bool enableInterrupt, int nSamples= 3) {
	...
	HAL_StatusTypeDef getGyroData(double *rotAccelVect, bool enableInterrupt, int nSamples= 3) {
		I2C_HandleTypeDef *bus= getI2CInterface();
		HAL_StatusTypeDef status=HAL_ERROR;
		vector<double> xGyro;
		vector<double> yGyro;
...
public:
	ImuManager(int bufferSize) : SensorManager(bufferSize) {
