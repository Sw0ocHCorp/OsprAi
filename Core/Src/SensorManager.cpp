...
void SensorManager::setI2CInterface(I2C_HandleTypeDef *i2cInterface) {
	this->i2cInterface= i2cInterface;
}

I2C_HandleTypeDef *SensorManager::getI2CInterface() {
	return this->i2cInterface;
}

void SensorManager::setUARTInterface(UART_HandleTypeDef *uartInterface) {
	this->uartInterface= uartInterface;
}

UART_HandleTypeDef *SensorManager::getUARTInterface() {
...
void SensorManager::setSeparator(char separator) {
...
void SensorManager::setParsingIds(map<uint16_t, uint16_t> parsingIds) {
	this->parsingIds= parsingIds;
}
