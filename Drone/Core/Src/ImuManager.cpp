...
HAL_StatusTypeDef ImuManager::SensorConfiguration(map<uint8_t, vector<uint8_t>> *configuration) {
		...
		HAL_StatusTypeDef status= HAL_I2C_Mem_Read(bus, MPU_SLAVE_ADDR, 0x75, 1, &address, 1, 1000);
		...
		if (status != HAL_ERROR && address >= 0x68 && address <= 0x71) {
			if (configuration == nullptr) {
				...
				if (configVal == 0b00000000)
					...
					this->gyroRange= 2000;
				status = HAL_I2C_Mem_Write(bus, MPU_SLAVE_ADDR, 0x1B, 1, &configVal, 1, 1000);
				//Accelero Config
				...
				if (configVal == 0b00000000)
					...
					this->accelRange= 32;
				status = HAL_I2C_Mem_Write(bus, MPU_SLAVE_ADDR, 0x1C, 1, &configVal, 1, 1000);
				//Wake UP du capteur
				...
				status = HAL_I2C_Mem_Write(bus, MPU_SLAVE_ADDR, 0x6B, 1, &configVal, 1, 1000);
			}
			//ExtractData(false);
		}
...
HAL_StatusTypeDef ImuManager::getGyroData(double *rotAccelVect, bool enableInterrupt= false) {
	I2C_HandleTypeDef *bus= getI2CInterface();
	uint8_t rawGyro[6];
	HAL_StatusTypeDef status=HAL_ERROR;
	int16_t signGyro [3];
	if (enableInterrupt) {
		status= HAL_I2C_Mem_Read_IT(bus, MPU_SLAVE_ADDR, 0x43, 1, rawGyro, 6);
	}
	else {
		status= HAL_I2C_Mem_Read(bus, MPU_SLAVE_ADDR, 0x43, 1, rawGyro, 6, 1000);
	}
...
HAL_StatusTypeDef ImuManager::getAccelData(double *linAccelVect, bool enableInterrupt= false) {
	I2C_HandleTypeDef *bus= getI2CInterface();
	uint8_t rawAccel[6];
	int16_t signAccel[3];
	HAL_StatusTypeDef status=HAL_ERROR;
	if (enableInterrupt) {
		status= HAL_I2C_Mem_Read_IT(bus, MPU_SLAVE_ADDR, 0x3B, 1, rawAccel, 6);
	}
	else {
		status= HAL_I2C_Mem_Read(bus, MPU_SLAVE_ADDR, 0x3B, 1, rawAccel, 6, 1000);
	}
